...
export function Feedback() {
  ...
  useEffect(() => {
    ...
    if (item) setPrevious(JSON.parse(item));
  }, [url]);
  ...
  function submit(e?: SyntheticEvent) {
    ...
    startTransition(async () => {
      const feedback: FeedbackData = { opinion, message, url, slackId };
      ...
      <CollapsibleContent className="mt-3">
        {previous ? (
          <div className="px-3 py-6 flex flex-col items-center gap-3 bg-fd-card text-fd-muted-foreground text-sm text-center rounded-xl">
            <p>Thanks! Your feedback was sent to our Slack.</p>
            <button
          ...
          <form className="flex flex-col gap-3" onSubmit={submit}>
            ...
            <input
              type="text"
              placeholder="Your Slack ID (for pinging)"
              value={slackId}
              onChange={(e) => setSlackId(e.target.value)}
              className="border rounded-lg bg-fd-secondary text-fd-secondary-foreground p-3 focus-visible:outline-none placeholder:text-fd-muted-foreground"
            />
            <button
              type="submit"
